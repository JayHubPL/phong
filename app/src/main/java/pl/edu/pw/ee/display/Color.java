...
public class Color {
    ...
    public static Color fromIllumination(Vector illumination) {
        return new Color(
            (int)(Math.max(0., Math.min(1., illumination.x())) * 255),
            (int)(Math.max(0., Math.min(1., illumination.y())) * 255),
            (int)(Math.max(0., Math.min(1., illumination.z())) * 255)
        );
    }

    public int toIntARGB(boolean isTransparent) {
        return ((isTransparent ? 0 : 255) << 24) | (red << 16) | (green << 8) | blue;
    }

    public int toIntARGB() {
        return toIntARGB(false);
    }
